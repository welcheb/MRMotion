function [filename] = openfile(directory,filterstring)
...
if nargin<2,
   filterstring = '*';
	...
	[ud.filterstring, ud.filter] = openfileGetfilterstring(filterstring);

	[ud.names, ud.isdirs] = openfileGetfilenames(ud.directory, ud.filter);
	...
	space = 25; 
    	...
    	uicontrol('style','text','string','Open File',...
...
function openfileAction(action)
...
switch action
	...
	case 2 % Filter Box Change
		...
		[ud.filterstring, ud.filter] = openfileGetfilterstring(newstring);
		[ud.names, ud.isdirs] = openfileGetfilenames(ud.directory, ud.filter);
...
function[filterstring, filter] = openfileGetfilterstring(newstring)
